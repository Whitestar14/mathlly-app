<template>
  <DialogRoot :open="open" @update:open="$emit('update:open', $event)">
    <Transition name="fade">
      <DialogOverlay class="fixed inset-0 bg-black/50 backdrop-blur-sm z-30" @click="$emit('update:open', false)" />
    </Transition>

    <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0 scale-95"
      enter-to-class="opacity-100 scale-100" leave-active-class="transition duration-150 ease-in"
      leave-from-class="opacity-100 scale-100" leave-to-class="opacity-0 scale-95">
      <DialogContent class="fixed inset-0 flex items-center justify-center p-4 z-30" aria-describedby="modal">
        <div
          ...
          <!-- Close Button -->
          <Button variant="ghost" size="icon" class="absolute right-4 top-4 dark:text-gray-100 p-1"
