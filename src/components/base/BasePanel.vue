<template>
  <div class="panel-container" :class="{ mobile: isMobile, 'panel-side': position === 'side' }">
    <!-- Backdrop (mobile only) -->
    <Transition name="fade">
      <div v-if="(isMobile || (position === 'side' && isMobile)) && isOpen" 
    ...
    <Transition :name="positionSide === 'left' ? 'slide-left' : 'slide-right'">
      <div v-if="position === 'side'" v-show="isOpen"
           class="side-panel-container"
           :class="[
             isMobile ? 'w-full' : `w-${sideWidth}`,
    ...
    <Transition name="slide-out">
      <div v-if="!isMobile && position !== 'side'" 
           class="desktop-panel" 
           :class="['transition-all duration-300 ease-in-out border-l border-gray-200 dark:border-gray-700']"
           :style="{ width: isOpen ? width : closedWidth }">
        <div class="panel-content absolute inset-y-0 right-0 bg-white dark:bg-gray-800 transition-opacity duration-300 max-h-[100vh]" 
             :class="[isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none', mainClass]">
          <PanelContent 
            :title="title" 
            :show-header="showHeader" 
            :show-footer="showFooter" 
    ...
    <Transition name="slide-up">
      <div v-if="isMobile && position !== 'side' && isOpen"
...
const props = defineProps({
  ...
  sideWidth: { type: String, default: "64" } // Width for side panel (e.g., '64' for w-64)
})
...
.side-panel-container {
  @apply fixed top-0 bottom-0 z-50 inset-y-0
  bg-gray-50 dark:bg-gray-900 transition-all;
  height: 100vh;
  overflow: hidden;
  display: flex;
  flex-direction: column;
