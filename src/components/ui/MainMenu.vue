<template>
      ...
      <PopoverPortal>
        <PopoverContent
          ...
          <a href="https://x.com/@SOlusunmbola" target="_blank"
            ...
            <ArrowUpRight class="h-4 w-4 mr-2 text-gray-500" />
            <span>Follow my Twitter</span>
          </a>

          <Separator orientation="horizontal" class="h-px w-full my-1 bg-gray-200 dark:bg-gray-700" />
  ...
  <div v-else>
    <div class="flex items-center space-x-2">
      <a href="https://github.com/Whitestar14/mathlly-app" target="_blank" rel="noopener noreferrer"
        aria-label="Github Page Link">
        <Button v-tippy="{ content: 'Star on GitHub', placement: 'bottom' }" variant="ghost" size="icon">
          <GithubIcon class="h-5 w-5" />
          <span class="sr-only">Star on GitHub</span>
        </Button>
      </a>

      <Button v-tippy="{ content: 'Keyboard Shortcuts (⌘K)', placement: 'bottom' }" variant="ghost" size="icon"
        @click="$emit('open-shortcut-modal')">
        <Command class="h-5 w-5" />
        <span class="sr-only">Keyboard Shortcuts</span>
      </Button>

      <Separator orientation="vertical" class="h-6 w-px bg-gray-200 dark:bg-gray-700" />
      <DropdownMenuRoot>
        <DropdownMenuTrigger as-child>
          <Button v-tippy="{ content: 'Change Theme', placement: 'bottom' }" variant="ghost" size="icon">
