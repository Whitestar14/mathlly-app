  ...
  <DialogRoot :open="open" @update:open="$emit('update:open', $event)">
    ...
    >
      ...
      >
        ...
        <button
          @click="closeModal"
          class="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:pointer-events-none"
        >
          <XIcon class="h-4 w-4" />
        ...
        <ul class="space-y-3">
          ...
          >
            <span 
              class="text-sm text-gray-600 dark:text-gray-300 mb-1 sm:mb-0"
            >
            ...
            >
              <template v-for="(modifier, modIndex) in shortcut.modifiers" :key="modIndex">
                <kbd 
                ...
                <span v-if="modIndex < shortcut.modifiers.length - 1" class="text-indigo-500 dark:text-indigo-400">+</span>
              </template>
              <kbd 
                class="bg-indigo-200 dark:bg-indigo-800 px-2 py-1 rounded text-xs font-semibold text-indigo-800 dark:text-indigo-200 shadow"
              >
