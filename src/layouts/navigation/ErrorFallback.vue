<template>
  ...
  >
    <div class="space-y-6 max-w-lg">
      <h1
        class="text-sm uppercase font-bold tracking-wider text-gray-500 dark:text-gray-400"
      >
        {{ headerTitle }}
      </h1>
      ...
      <div class="relative">
        <h1
          class="text-9xl font-bold text-gray-200 dark:text-gray-700 select-none"
        >
      ...
      <div class="space-y-2">
        ...
        <p
          v-if="isOffline && autoRetryEnabled"
      ...
      <div
        class="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2 *:min-w-[120px]"
      >
        <Button
          v-if="isRouteError || isOffline"
          variant="ghost"
          class="bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600"
          @click="retry"
          :disabled="isRetrying || (isOffline && !networkStatus)"
          :aria-label="
            isOffline ? 'Retry Connection' : 'Try Loading Page Again'
          "
        >
...
const messageBody = computed(() => {
  ...
  if (errorCode.value === '404')
    return `We couldn't find the page at ${
      props.path ? `\`${props.path}\`` : 'the requested URL'
    }.`;
