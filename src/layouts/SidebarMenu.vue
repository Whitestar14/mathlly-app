<template>
  <BasePanel :is-open="isOpen" :is-mobile="isMobile" position="side" position-side="left" sideWidth="64"
    ...
    <div class="flex flex-col h-full">
      <!-- Logo header -->
      <div class="flex flex-none items-center justify-between border-b border-gray-200 dark:border-gray-700 h-14">
        <div class="size-full px-2 py-1 relative">
          <div class="flex items-center justify-between md:justify-around">
            <Logo />
            <Button v-tippy="{ content: 'Close Sidebar', placement: 'bottom' }" variant="ghost" size="icon"
              :class="{ 'hidden': !isOpen }" @click="closeSidebar">
              <PanelLeftIcon class="h-5 w-5" />
            </Button>
          </div>
        </div>
          ...
          <NavigationMenuList class="px-3 py-2 space-y-6">
            ...
            <div v-for="category in categories" :key="category.title" class="space-y-2">
              ...
              <div class="space-y-0.5">
                <NavigationMenuItem v-for="item in category.items" :key="item.path" class="space-y-0.5 z-0">
      ...
      <!-- Footer -->
      <div
        class="flex-none flex-shrink-0 p-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
